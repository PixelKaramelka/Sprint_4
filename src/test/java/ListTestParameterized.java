...
//Проверка, что появляется выпадающий список при нажатии на стрелку
...
public class ListTestParameterized {
    ...
    public void setUp(){
        ...
        driver.manage().window().maximize(); //полноэкранный режим браузера
        mainPage.clickButtonCookie(); //клик на кнопку куки
        mainPage.scrollToTable();
    }
    //клик по кнопке списка и проверка выпадающего текста
    @Test
    public void testAccordingDropdown(){
        mainPage.clickClosedDropDownByIndex(index);
        assertEquals(expected,
                mainPage.getTextFromTextInputByIndex(index));
    }
